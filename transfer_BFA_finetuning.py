...
from utils import AverageMeter, RecorderMeter, time_string, change_quan_bitwidth
...
def main():
    ...
    all_param = [
        param for name, param in list(classifier_teacher.named_parameters()) if not 'step_size' in name
        if not 'step_size' in name
    ]

    step_param = [
        param for name, param in list(feature_extractor.named_parameters())+list(classifier_teacher.named_parameters()) if 'step_size' in name 
    ]
    ...
    # separate the parameters thus param groups can be updated by different optimizer
    std_all_param = [
        param for name, param in classifier_student.named_parameters()
        if not 'step_size' in name
    ]
    ...
    if args.optimizer == "SGD":
        print("using SGD as optimizer")
        std_optimizer = torch.optim.SGD(std_all_param,
                                    lr=state['learning_rate'],
                                    momentum=state['momentum'],
                                    weight_decay=state['decay'],
                                    nesterov=True)
    ...
    if args.enable_bfa:
        perform_attack(attacker, feature_extractor, classifier_student, net_clean_student, train_loader_student, test_loader_student,
                    ...
                    args.n_iter, criterion, optimizer, std_optimizer, log, writer, csv_save_path=args.save_path,
...
def init_student_dataset(args):
    ...
    # number of percentage of data is given by args.percent
    if not os.path.isdir(args.data_path):
        os.makedirs(args.data_path)
...
def perform_attack(attacker, feature_extractor, classifier_student, model_student_clean, train_loader_student, test_loader_student,
                   ...
                   N_iter, criterion, optimizer, std_optimizer, log, writer, csv_save_path=None, random_attack=False):
    ...
    for i_iter in range(N_iter):
        ...
        # After search the attacked bit, freeze the bit and finetune the model on source domain.
        if i_iter % 2 == 0:
            feature_extractor_to_train_teacher = copy.deepcopy(feature_extractor)
            feature_extractor_to_train_student = copy.deepcopy(feature_extractor)
            classifier_student_original = copy.deepcopy(classifier_student)
            classifier_teacher_original = copy.deepcopy(classifier_teacher)
            classifier_teacher, _, _, = finetune_feature_extractor(feature_extractor_to_train_teacher, classifier_teacher, criterion, optimizer, log, 
                                    train_loader_teacher, test_loader_teacher, epochs=10, verbose=False)
            classifier_student, _, _, = finetune_student_classifier(feature_extractor_to_train_student, classifier_student, criterion, std_optimizer, log, 
                                    train_loader_student, test_loader_student, epochs=10, verbose=False)
            # Check if classifier student and teacher are updated
            # Check if the teacher classifier has been updated
            teacher_diff = hamming_distance(classifier_teacher, classifier_teacher_original)
            print(f'Teacher difference: {teacher_diff}')
            # Check if the student classifier has been updated
            student_diff = hamming_distance(classifier_student, classifier_student_original)
...
def finetune_feature_extractor(feature_extractor, classifier, criterion, optimizer, log, train_loader, test_loader, epochs=5, verbose=True):
    ...
    trainable_param = [
        param for name, param in classifier.named_parameters()
        if not 'step_size' in name
    ]
    ...
    classifier.train()
    # optimizer = torch.optim.SGD(classifier.parameters(), 0.1, momentum=0.9, weight_decay=1e-4)
    ...
    for epoch in range(epochs):
        for i, (input, target) in enumerate(train_loader):
            ...
            loss = criterion(output, target)  # Original loss function
    ...
    for m in feature_extractor.modules():
        if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
            m.__reset_weight__()
    return classifier, top1.avg, losses.avg

def finetune_student_classifier(feature_extractor, classifier, criterion, optimizer, log, train_loader, test_loader, epochs=5, verbose=True):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
